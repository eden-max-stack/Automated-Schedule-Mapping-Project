...
const createTemplate = async (req, res) => {
    ...
    try {
        const [rows] = await db.execute('SELECT * FROM templates WHERE template_title = ?', [templateName]);
        if (rows.length > 0) return res.status(409).json({'message' : 'Template name already exists! Choose a different one.'}); // conflict
        ...
        const templateTable = await db.execute(`CREATE TABLE ${templateName} (Name varchar(255), Description varchar(255), subjCode char(1), startDate datetime, endDate datetime, Colour int, DayOrder int)`);
